...
| POST | `/enrollment-requests` | Public enrollment request. | Visitor | None | fullName, email, optional phone/branchId/preferredBranch/styleInterest/message | `201` request | Email format and name length validated. If branchId is provided it must exist; preferredBranch is normalized as a fallback. | `404`, `422` |
...
| POST | `/users` | Create an academy user with a temporary password. | GeneralDirector, Admin | None | email, name, role, optional temporaryPassword, active, branchIds, studentProfile, teacherProfile | `201` user, linked profile when applicable, and one-time temporary password | Stores only a password hash. New users always require first password change. Student and Teacher accounts require a linked profile payload. BranchDirector accounts require at least one branch ID. Only Admin can create Admin accounts. Audited. | `401`, `403`, `404`, `409`, `422` |
...
| PATCH | `/users/{id}/branch-access` | Replace branch access assigned to a user. | GeneralDirector, Admin | Path `id` | `{ "branchIds": ["uuid"] }` | `200` list | Target user must be BranchDirector and branch IDs must exist. Audited. | `401`, `403`, `404`, `422` |
...
| POST | `/absence-justifications` | Submit absence justification. | Student, Teacher, directors/admin | None | attendanceRecordId, reason, evidenceUrl | `201` record | Attendance record must exist and currently be absent. | `401`, `403`, `404`, `422` |
| PATCH | `/absence-justifications/{id}/review` | Approve/reject justification. | BranchDirector, GeneralDirector, Admin | Path `id` | status, reviewNotes | `200` record | Pending reviews only. Approval updates the attendance record status to justified. Audited. | `401`, `403`, `404`, `409`, `422` |
