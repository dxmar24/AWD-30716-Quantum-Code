...
describe('auth/session', () => {
  ...
  test('director can create an academy account and send the temporary password by email', async () => {
    const app = createApp();
    ...
    expect(created.body.data.invitation).toMatchObject({ status:'sent', recipient:'created.student@alc.edu' });
    const invitation = app.locals.emailService.latestInvitationFor('created.student@alc.edu');
    expect(invitation.temporaryPassword).toMatch(/^ALC-/);
    expect(invitation.html).toContain('American Latin Class');

    const login = await request(app)
      .post('/api/v1/auth/login')
      .send({ email:'created.student@alc.edu', password:invitation.temporaryPassword })
      .expect(200);
    expect(login.body.data.user.mustChangePassword).toBe(true);
  });
