...
const source = () => ({
  ...
  students:[
    { id:'student-1', fullName:'Ana Torres', branchId:'north', level:'B1', active:true },
    { id:'student-2', fullName:'Bruno Vega', branchId:'north', level:'B2', active:false },
    { id:'student-3', fullName:'Camila Paz', branchId:'south', level:'B2', active:true },
  ],
  payments:[
  ...
  classGroups:[
    { id:'group-north', name:'Salsa Level 1', branchId:'north', level:'B1', active:true, capacity:20 },
    { id:'group-south', name:'Hip Hop Foundation', branchId:'south', level:'B2', active:true, capacity:20 },
  ],
  classSessions:[
    { id:'session-north', classGroupId:'group-north', startsAt:'2026-07-08T22:00:00.000Z' },
    { id:'session-south', classGroupId:'group-south', startsAt:'2026-07-09T22:00:00.000Z' },
  ],
  enrollments:[
    { id:'enrollment-1', studentId:'student-1', classGroupId:'group-north', status:'active' },
    { id:'enrollment-2', studentId:'student-2', classGroupId:'group-north', status:'active' },
    { id:'enrollment-3', studentId:'student-3', classGroupId:'group-south', status:'active' },
  ],
  justifications:[],
  teachers:[],
  teacherAttendance:[],
...
test('functional helpers receive behavior as parameters', () => {
  ...
  expect([...groupBy(payments, (payment) => payment.branchId).keys()]).toEqual(['north', 'south']);
});
...
test('report filters combine branch, level, payment status and normalized student search', () => {
  const filtered = applyAcademicReportFilters(source(), {
    branchId:'north',
    level:'B1',
    paymentStatus:'paid',
    search:'ana',
  ...
  expect(filtered.branches.map((item) => item.id)).toEqual(['north']);
  expect(filtered.students.map((item) => item.id)).toEqual(['student-1']);
  expect(filtered.payments.map((item) => item.id)).toEqual(['payment-1']);
  expect(filtered.attendance.map((item) => item.id)).toEqual(['attendance-1']);
});

test('attendance report groups evidence by student, class and branch without hiding justified absences', () => {
  const data = source();
  data.justifications = [{ attendanceRecordId:'attendance-2', status:'approved' }];
  const report = buildAttendanceReport(
    data,
    '2026-07-10T12:00:00.000Z',
    { from:'2026-07-01T00:00:00.000Z', to:'2026-07-10T23:59:59.000Z' },
  );
  ...
  expect(report.totals).toMatchObject({
    ...
    totalSessions:3,
    excusedAbsences:1,
    rawAttendanceRate:66.67,
    attendanceRate:100,
  });
  expect(report.byStudent.map((item) => item.name)).toEqual(['Ana Torres', 'Bruno Vega', 'Camila Paz']);
  expect(report.byClassGroup).toHaveLength(2);
  expect(report.byBranch).toHaveLength(2);
  expect(report.records.find((item) => item.id === 'attendance-2')).toMatchObject({
    studentName:'Bruno Vega',
    classGroupName:'Salsa Level 1',
    justified:true,
  });
});

test('attendance report supports focused status and student filters', () => {
  const data = source();
  data.justifications = [{ attendanceRecordId:'attendance-2', status:'approved' }];

  const report = buildAttendanceReport(data, '2026-07-10T12:00:00.000Z', null, {
    attendanceStatus:'justified',
    studentId:'student-2',
  });

  expect(report.records).toHaveLength(1);
  expect(report.records[0]).toMatchObject({ id:'attendance-2', justified:true });
});
