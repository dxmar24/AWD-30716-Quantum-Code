...
test('an erroneous enrollment can be deleted only before attendance history exists', async () => {
  const app = createApp();
  const cookie = await adminCookie(app);
  const secondStudent = app.locals.db.students.create({
    branchId:IDS.branch,
    fullName:'Enrollment Correction Student',
    level:'B1',
    active:true,
  });
  ...
  await request(app)
    .delete(`/api/v1/class-group-enrollments/${created.body.data.id}`)
    .set('Cookie', cookie)
    .expect(200);
  expect(app.locals.db.classGroupEnrollments.findById(created.body.data.id)).toBeNull();
  expect(app.locals.db.auditLogs.all()).toEqual(expect.arrayContaining([
    expect.objectContaining({ action:'CLASS_GROUP_ENROLLMENT_DELETED', entityId:created.body.data.id }),
  ]));
  ...
  const rejected = await request(app)
    .delete(`/api/v1/class-group-enrollments/${IDS.enrollment}`)
    .set('Cookie', cookie)
    .expect(409);
  expect(rejected.body.details).toMatchObject({ code:'ENROLLMENT_HAS_ATTENDANCE_HISTORY' });
  expect(app.locals.db.classGroupEnrollments.findById(IDS.enrollment)).not.toBeNull();
});
