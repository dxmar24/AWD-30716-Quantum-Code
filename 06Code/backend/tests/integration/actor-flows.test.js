...
describe('actor flow verification', () => {
  ...
  test('Student flow: first password change, own academic data and denied restricted actions', async () => {
    ...
    await request(app).get('/api/v1/reports/attendance').set('Authorization', `Bearer ${firstLogin.token}`).expect(403);
  ...
  test('Teacher flow: own classes, attendance work and restricted administration', async () => {
    ...
    await request(app).get('/api/v1/reports/attendance').set('Cookie', teacher.cookie).expect(403);
  ...
  test('BranchDirector flow: assigned branch management and cross-branch denial', async () => {
    ...
    const attendanceReport = await request(app)
      .get(`/api/v1/reports/attendance?branchId=${ids.northBranch}`)
      .set('Cookie', director.cookie)
      .expect(200);
    expect(attendanceReport.body.data.filters.branchId).toBe(ids.northBranch);
    await request(app)
      .get(`/api/v1/reports/attendance?branchId=${ids.centralBranch}`)
      .set('Cookie', director.cookie)
      .expect(404);
  ...
  test('GeneralDirector flow: academy account creation, branch access and global reports', async () => {
    ...
    const attendanceReport = await request(app)
      .get('/api/v1/reports/attendance?level=B1&attendanceStatus=present')
      .set('Cookie', generalDirector.cookie)
      .expect(200);
    expect(attendanceReport.body.data.filters).toMatchObject({ level:'B1', attendanceStatus:'present' });
    await request(app)
      .get('/api/v1/reports/attendance?level=B3')
      .set('Cookie', generalDirector.cookie)
      .expect(422);
    await request(app)
      .get('/api/v1/reports/attendance?studentId=not-an-id')
      .set('Cookie', generalDirector.cookie)
      .expect(422);
    ...
    expect(createdStudent.body.data.temporaryPassword).toBeUndefined();
    const studentInvitation = app.locals.emailService.latestInvitationFor('student.created.by.gd@alc.test');
    const studentLogin = await loginWithPassword(app, 'student.created.by.gd@alc.test', studentInvitation.temporaryPassword);
    const changedStudentPassword = await request(app)
      .post('/api/v1/auth/change-password')
      .set('Authorization', `Bearer ${studentLogin.token}`)
      .send({ currentPassword:studentInvitation.temporaryPassword, newPassword:'studentCreatedALC2027*' })
      .expect(200);
    ...
    expect(app.locals.emailService.latestInvitationFor('branch.director.created@alc.test').temporaryPassword).toMatch(/^ALC-/);
    ...
    await request(app)
      ...
      .expect(403);
    await request(app)
      .post(`/api/v1/users/${created.body.data.user.id}/reset-password`)
      .set('Cookie', generalDirector.cookie)
      .expect(403);
  });
  ...
  test('Admin flow: role governance, branch access governance and full audit visibility', async () => {
    ...
    await request(app)
      ...
      .expect(200);
    await request(app).get('/api/v1/branches').set('Cookie', admin.cookie).expect(200);
    ...
    const reset = await request(app)
      ...
      .set('Cookie', admin.cookie)
      .expect(200);
    expect(reset.body.data.temporaryPassword).toBeUndefined();
    expect(reset.body.data.user.mustChangePassword).toBe(true);
    const resetInvitation = app.locals.emailService.latestInvitationFor(targetUser.email);
    expect(resetInvitation.temporaryPassword).toMatch(/^ALC-/);

    const targetLogin = await loginWithPassword(app, targetUser.email, resetInvitation.temporaryPassword);
    await request(app)
      .patch(`/api/v1/users/${targetUser.id}/status`)
      .set('Cookie', admin.cookie)
      .send({ active:false })
      .expect(200);
    await request(app).get('/api/v1/auth/me').set('Cookie', targetLogin.cookie).expect(401);
    await request(app)
      .patch(`/api/v1/users/${targetUser.id}/status`)
      .set('Cookie', admin.cookie)
      .send({ active:true })
    ...
    await request(app)
      .patch(`/api/v1/users/${admin.user.id}/role`)
      .set('Cookie', admin.cookie)
      .send({ role:Roles.GENERAL_DIRECTOR })
      .expect(422);
    await request(app)
      .post(`/api/v1/users/${admin.user.id}/reset-password`)
      .set('Cookie', admin.cookie)
      .expect(422);
