...
class AuthController { constructor(authService, db = null) { this.authService = authService; this.db = db; }
  ...
  me = async (req, res) => {
    let profilePhotoUrl = '';
    if (this.db && req.sessionUser?.role === 'Student') {
      const profile = await this.db.students.findBy('userId', req.sessionUser.id);
      profilePhotoUrl = profile?.profilePhotoUrl || '';
    }
    return ApiResponse.success(res, { user:{ ...req.sessionUser, profilePhotoUrl } });
  };
