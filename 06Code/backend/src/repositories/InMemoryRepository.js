...
class InMemoryRepository {
  ...
  findByFields(fields) { return this.rows.find((row) => Object.entries(fields).every(([field, value]) => row[field] === value)) || null; }
  ...
  deleteById(id) { const row = this.findById(id); if (!row) return null; this.rows = this.rows.filter((item) => item.id !== id); return row; }
