...
class AttendanceService {
  ...
  async deleteEnrollment(actor, id) {
    const existing = await this.db.classGroupEnrollments.findById(id);
    if (!existing) throw new AppError('Class group enrollment not found', 404);
    if (this.accessPolicy) await this.accessPolicy.assertCanUpdate(actor, 'class-group-enrollments', existing, { delete:true });

    const removed = await this.inTransaction(async (db) => {
      const current = await db.classGroupEnrollments.findById(id);
      if (!current) throw new AppError('Class group enrollment not found', 404);
      const sessions = (await db.classSessions.all()).filter((session) => (
        session.classGroupId === current.classGroupId && enrollmentCoversSession(current, session)
      ));
      const sessionIds = new Set(sessions.map((session) => session.id));
      const attendanceHistory = (await db.studentAttendance.all()).filter((row) => (
        row.studentId === current.studentId && sessionIds.has(row.classSessionId)
      ));
      if (attendanceHistory.length) {
        throw new AppError('Enrollment cannot be deleted because attendance history already exists. Mark it as withdrawn or completed instead.', 409, {
          code:'ENROLLMENT_HAS_ATTENDANCE_HISTORY',
          attendanceRecords:attendanceHistory.length,
        });
      }
      const row = await db.classGroupEnrollments.deleteById(id);
      await this.auditWith(db, actor.id, 'CLASS_GROUP_ENROLLMENT_DELETED', 'class_group_enrollments', id, {
        deletedEnrollment:current,
        reason:'Erroneous enrollment without attendance history',
      });
      return row;
    });
    this.invalidateAttendanceCaches(['students']);
