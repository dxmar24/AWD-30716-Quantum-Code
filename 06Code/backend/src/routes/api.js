...
const { uploadAbsenceEvidence } = require('../middleware/evidenceUpload');
...
function buildApi(deps) {
  ...
  const auth = new AuthController(deps.authService, deps.db);
  ...
  const evidenceUploadLimiter = rateLimit({ windowMs: 15*60*1000, max: 20 });
  ...
  router.patch('/users/:id/status', allowRoles(Roles.ADMIN), validate(userStatusUpdate), wrap(academic.updateUserStatus));
  router.post('/users/:id/reset-password', allowRoles(Roles.ADMIN), wrap(academic.resetUserPassword));
  router.post('/users/:id/resend-invitation', allowRoles(Roles.ADMIN, Roles.GENERAL_DIRECTOR), wrap(academic.resendUserInvitation));
  ...
  router.delete('/class-group-enrollments/:id', allowRoles(...directorRoles), wrap(attendance.deleteEnrollment));
  ...
  router.post('/absence-justifications', allowRoles(Roles.ADMIN, Roles.GENERAL_DIRECTOR, Roles.BRANCH_DIRECTOR, Roles.TEACHER, Roles.STUDENT), evidenceUploadLimiter, uploadAbsenceEvidence, validate(absenceJustification), wrap(academic.createAbsenceJustification));
  router.get('/absence-justifications/:id/evidence', requireAuth, requirePasswordReady, wrap(academic.getAbsenceEvidence));
  ...
  router.get('/reports/attendance', allowRoles(...directorRoles), privateCache(30, 'private-scoped-report-cache'), wrap(reports.attendanceReport));
