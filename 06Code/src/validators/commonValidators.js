...
), 'Profile photo must be an image data URL or an HTTP URL');
...
const classSession = z.object({ classGroupId:z.string().uuid(), name:z.string().min(2).max(120).optional(), startsAt:z.string().datetime(), endsAt:z.string().datetime(), status:z.enum(['scheduled','completed','cancelled']).optional() });
const academyEvent = z.object({ branchId:z.string().uuid(), title:z.string().min(3).max(160), description:z.string().max(1000).optional(), level:z.enum(['B1','B2','ALL']).default('ALL'), startsAt:z.string().datetime(), endsAt:z.string().datetime().optional(), location:z.string().max(160).optional(), showIncome:z.coerce.number().min(0).optional(), active:z.boolean().optional() });
const studentPayment = z.object({ studentId:z.string().uuid(), branchId:z.string().uuid(), amount:z.coerce.number().min(0), concept:z.string().min(3).max(100), period:z.string().regex(/^\d{4}-\d{2}$/), status:z.enum(['paid','pending','overdue','cancelled']).default('pending'), paidAt:z.string().datetime().nullable().optional(), dueAt:z.string().datetime().nullable().optional(), notes:z.string().max(500).optional() });
module.exports = { z, idParam, attendanceRecord, teacherCheck, roleUpdate, accountCreate, passwordChange, branchAccessUpdate, enrollmentRequest, absenceJustification, absenceReview, scholarshipEvaluation, levelPromotionEvaluation, branch, student, studentPhoto, teacher, danceCategory, danceStyle, classGroup, classSession, academyEvent, studentPayment };
