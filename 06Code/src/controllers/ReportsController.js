...
class ReportsController {
  ...
  async buildReport(user, period) {
    const generatedAt = new Date();
    const asOf = reportAsOf(period, generatedAt);
    return buildAcademicReport(await this.reportSource(user, period), generatedAt, period, asOf);
  }

  cachedReport = async (req, res, keySuffix, loader, notFoundMessage = null) => {
    const queryKey = `${req.query.from || 'all'}:${req.query.to || 'all'}`;
    let value;
  ...
  branchSummary = async (req, res) => {
    ...
    return this.cachedReport(req, res, 'branches:summary', async () => {
      const report = await this.buildReport(req.sessionUser, period);
      return {
  ...
  generalReport = async (req, res) => {
    const period = reportPeriod(req.query);
    return this.cachedReport(req, res, 'general', () => this.buildReport(req.sessionUser, period));
  };
  ...
  branchDetail = async (req, res) => {
    ...
    return this.cachedReport(req, res, `branches:${req.params.branchId}:detail`, async () => {
      ...
      const generatedAt = new Date();
      const report = buildAcademicReport(branchSource, generatedAt, period, reportAsOf(period, generatedAt));
      return {
        generatedAt:report.generatedAt,
        asOf:report.asOf,
        period:report.period,
...
module.exports = { ReportsController, reportAsOf, reportPeriod, withinPeriod };
