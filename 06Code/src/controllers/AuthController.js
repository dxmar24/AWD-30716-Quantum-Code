...
class AuthController { constructor(authService) { this.authService = authService; }
  ...
  sendLoginResponse(res, token, user) {
    ...
    return ApiResponse.success(res, { sessionToken:token, tokenType:'Bearer', expiresInMinutes:env.sessionTtlMinutes, user:{ id:user.id, email:user.email, name:user.name, role:user.role, mustChangePassword:Boolean(user.mustChangePassword) } }, 'Login successful');
  }
  ...
  changePassword = async (req, res, next) => { try { const user = await this.authService.changePassword(req.sessionUser, req.body.currentPassword, req.body.newPassword); return ApiResponse.success(res, { user:{ id:user.id, email:user.email, name:user.name, role:user.role, mustChangePassword:Boolean(user.mustChangePassword) } }, 'Password changed'); } catch (e) { return next(e); } };
